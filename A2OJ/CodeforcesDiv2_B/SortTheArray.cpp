...
void check_array(int inp[], int n, int start, int end) {
	int check = 1;
	n--;
	forn(i, n) {
		if(inp[i] > inp[i + 1]) {
	...
	if (check) cout << "yes" << ln << (start + 1) << " " << (end + 1) << ln;
	else cout << "no";
}

void reverse_segment(int inp[], int start, int end) {
	int temp;
	forsn(i, start, end) {
		if(start <= end) {
			temp = inp[start];
...
void find_end(int inp[], int n, int start) {
	int end = n - 1;
	forsn(i, start, end) {
		if (inp[i] < inp[i + 1]) {
			end = i;
	...
	reverse_segment(inp, start, end);
	check_array(inp, n, start, end);
}
