...
int main() {
	...
	v32 val;
	forn(i, 26)
		cin >> temp, val.pb(temp);

	v32 inp_val;
	for(auto i : inp)
		inp_val.pb(val[(int)i - 97]);

	auto val_max = max_element(val.begin(), val.end());

	int64 ans = 0;
	for(int i = 0; i < inp.length(); i++)
		ans += inp_val[i]*(i+1);

	int64 range_val = ((double)((inp.length()+1)+(inp.length()+k))/2)*k;

	cout << (ans + *val_max*range_val);
