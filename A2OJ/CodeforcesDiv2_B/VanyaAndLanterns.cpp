...
int main() {
	...
	forn(i, n) cin >> k, inp.pb(k);
	sort(inp.begin(), inp.end());

	double max = inp[0];
	forsn(i, 1, n)
		max = (max > (inp[i] - inp[i - 1])/2) ? max : ((inp[i] - inp[i - 1])/2);
	max = (max > (l - inp[inp.size() - 1])) ? max : (l - inp[inp.size() - 1]);

	cout << fixed << showpoint << setprecision(10) << max << endl;
