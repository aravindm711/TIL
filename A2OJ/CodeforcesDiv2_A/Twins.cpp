...
int diff(int x, int sum) { return sum - x; }
...
int main() {
	...
	cin >> n;
	int sum = 0;
	int* inp_integers = new int[n];
	forn(i, n) {
		cin >> inp_integers[i];
		sum += inp_integers[i];
	}
	sort(inp_integers, inp_integers + n, greater<int>());

	int sum_temp = 0;
	forn(i, n) {
		sum_temp += inp_integers[i];
		if (sum_temp > ceil(sum / 2)) {
			cout << i + 1;
			break;
		}
	}
